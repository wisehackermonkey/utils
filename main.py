...
def parse_event_with_claude(text_content):
    """Use Claude to extract event details from text input"""
    ...
    response = client.messages.create(
        ...
        max_tokens=1000,
        temperature=0,
        system="You are an event information extraction assistant. Extract structured event data from text and return only valid JSON.",
        messages=[
    ...
    import json
    try:
        # Find JSON in response (might be wrapped in code blocks)
...
def main():
    ...
    # Extract event data with Claude
    print("Parsing event information...")
    event_data = parse_event_with_claude(input_text)
